...
[InvocationCount(1)]
public class SelectWithMatchPatternComparison
{
    ...
    [Benchmark]
    public void PostgresqlSelectMatchPattern()
    {
        _npgsqlConnection.Query("SELECT first_name FROM person where first_name LIKE 'Jo%'");
    }
    
    [Benchmark]
    public void RedisSelectMatchPattern()
    {
        _db.Execute("FT.SEARCH", "idx:person", "@FirstName:Jo*", "RETURN", "1", "FirstName", "LIMIT", 0, _people.Count);
    }
}
