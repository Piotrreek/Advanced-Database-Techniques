...
[InvocationCount(1)]
public class SelectWithIndexComparison
{
    ...
    [Benchmark]
    public void PostgresSelectWithIndex()
    {
        _npgsqlConnection.Query("SELECT * FROM person p WHERE p.first_name = 'Laura'");
    }
    
    [Benchmark]
    public void RedisSelectWithIndex()
    {
        _db.Execute("FT.SEARCH", "idx:person", "@FirstName:Laura", "LIMIT", "0", _people.Count);
    }
}
