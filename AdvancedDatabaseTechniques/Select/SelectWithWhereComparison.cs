...
[InvocationCount(1)]
public class SelectWithWhereComparison
{
    ...
    [Benchmark]
    public void PostgresSelectWithWhere()
    {
        _npgsqlConnection.Execute(
            "SELECT first_name FROM person p INNER JOIN job j ON j.person_id = p.id WHERE j.salary > 200");
    }
    
    [Benchmark]
    public void RedisSelectWithWhere()
    {
        var result = _db.Execute("FT.SEARCH", "idx:person", "FirstName", "LIMIT", "0", _people.Count);
        var persons = (RedisResult[])result;
        for (var i = 1; i < persons.Length; i+=2)
        {
            var key = (string)persons[i];
            var id = key.Split(":")[1];
            _db.Execute("FT.SEARCH", "idx:job", $"@PersonId:{id}", "@Salary:[200 +inf]");
        }
    }
}
