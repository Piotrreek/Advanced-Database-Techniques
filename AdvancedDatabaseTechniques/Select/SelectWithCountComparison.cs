...
[InvocationCount(1)]
public class SelectWithCountComparison
{
    ...
    private IBatch _batchInsertSet = default!;
    private readonly List<Task> _insertTasks = [];
    private readonly List<Task> _insertSetTasks = [];
    ...
    public void GlobalSetup()
    {
        ...
        _batchInsertSet = _db.CreateBatch();
        for (var i = 0; i < _people.Count; i++) 
        {
            ...
            _insertSetTasks.Add(_batchInsertSet.SetAddAsync("person_set", key));
        }
        _batchInsert.Execute();
        _batchInsertSet.Execute();
        Task.WaitAll(_insertTasks.ToArray());
        Task.WaitAll(_insertSetTasks.ToArray());
    }
    ...
    [Benchmark]
    public void PostgresSelectWithCount()
    {
        _npgsqlConnection.Query("SELECT COUNT(*) FROM person");
    }
    
    [Benchmark]
    public void RedisSelectWithCount()
    {
       _db.SetLength("person_set");
    }
}
